...
class BpmPreproc:
    def __init__(self):
        ...
        self.bpms: list = [BPM(bpm, self.collect_orbit, self.chan_tunes, self.chan_turns,
                               self.chan_fft, self.chan_coor, self.collect_act_bpm, CONF) for bpm in bpms_list]
    ...
    def collect_act_bpm(self):
        act_bpm_list = []
        for bpm in self.bpms:
            if bpm.act_state:
                act_bpm_list.append(bpm.name)
        self.chan_act_bpm.setValue(json.dumps(act_bpm_list))
        ...
        if permission:
            ...
            for bpm in self.bpms:
                ...
                if bpm.act_state:
                    ...
                    x_orbit_sigma = np.append(x_orbit_sigma, bpm.std[0])
                    z_orbit = np.append(z_orbit, bpm.coor[1])
                    z_orbit_sigma = np.append(z_orbit_sigma, bpm.std[1])
                else:
                    x_orbit = np.append(x_orbit, 100.0)
                    x_orbit_sigma = np.append(x_orbit_sigma, -1.0)
                    z_orbit = np.append(z_orbit, 100.0)
                    z_orbit_sigma = np.append(z_orbit_sigma, -1.0)
